...
> | **블록 접근의 64%가 TOAST 인덱스 조회** (cross-scoring이 진짜 비용) | **더는 아니다.** 퍼널(`2d66ea2`)+워밍 제거(`b020104`) 후 `clova_chunk_vectors`의 toast+tidx는 요청당 1,848블록 = **전체의 11%**다. 현재 지배 항목은 **binary HNSW 순회 43%**(`clova_article_chunk` idx 7,302블록)와 **중복 `JOIN article` 21%**(요청당 PK 조회 1,113회)다 |
> | A + B'로 이론 RPS 2.94 → 4.0~4.3 | 그 예측의 전제(요청당 비용)가 바뀌었으므로 **재계산 필요** |
>
> **여전히 유효한 것**: "꺼내는 벡터 개수를 줄여야 한다"는 결론(A 레버), 청크당 디토스트 비용의
> 구조적 성격, 그리고 요청당 블록 접근량이 병목이라는 진단 — 2026-08-17에도 여전히
> **16,903블록(132MB)/요청**이다. 다만 **그 블록의 소유자 구성이 바뀌었다**(위 표 마지막 줄).
> 관계 단위 귀속은 `pg_statio_user_tables`로 가능하다 — pgss 스냅샷 없이도 뜬다.
>
