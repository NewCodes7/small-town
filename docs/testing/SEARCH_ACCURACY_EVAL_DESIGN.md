...
불일치 26건 중 22건이 인접(±1)이라 "거의 맞음"으로 쳐주면 점수가 크게 오른다. 그러나 NDCG 의
gain 은 `2^등급 − 1`(0/1/3/7)이라 **2와 3의 차이는 3점 대 7점, 2배가 넘는다.** 인접 불일치는
"거의 맞음"이 아니다. (평균 등급 차도 실패했으므로 결론 자체는 κ 정의에 의존하지 않는다.)
