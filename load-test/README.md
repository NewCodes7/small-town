...
>
> ⚠️ **실행 창 주의**: 03:30(HN)/04:00(블로그)/04:30(유튜브)/05:00(본문 백필) 크롤링, 매시 :30 검색 프리워밍, blue/green 배포와 겹치지 않는 시간대에 실행한다. 컨테이너는 `/etc/timezone`을 호스트에서 마운트해 **KST로 돈다**(04:30 KST에 DB CPU 0.014 → 0.042로 뜨는 것으로 확인). 이전 문구의 "02:00/02:30"은 cron이 04:00/04:30(`application-prod.properties`)으로 바뀐 뒤 갱신되지 않은 값이었다.
