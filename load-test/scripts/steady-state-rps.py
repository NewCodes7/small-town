#!/usr/bin/env python3
"""판정 레벨(기본: 사다리의 두 번째로 높은 VU)의 과도구간(앞 60초)을 제외한 정상상태 처리량 비교.

사용법: python3 load-test/scripts/steady-state-rps.py <testid> [<testid2> ...]

VU 사다리와 판정 레벨은 VU_LEVELS / STEADY_LEVEL 환경변수로 오버라이드한다.

왜 필요한가: ramp-limit-finder는 VU를 계단식으로 올리는데, 요청 하나가
...
# scenarios/ramp-limit-finder.js의 기본 사다리와 맞춰야 한다.
LEVELS = [int(x) for x in os.environ.get("VU_LEVELS", "5,10,15,20").split(",")]
# 판정 레벨: 기본은 두 번째로 높은 레벨(최상단은 붕괴 구간이라 비교가 흔들린다)
STEADY_LEVEL = int(os.environ.get("STEADY_LEVEL", LEVELS[-2]))
STEADY_INDEX = LEVELS.index(STEADY_LEVEL)
def token():
...
for t in sys.argv[1:]:
    ...
    ls = b + STEADY_INDEX*210            # 판정 레벨 시작
    sel = f'{{testid="{t}",level="{STEADY_LEVEL}",class="2xx"}}'
