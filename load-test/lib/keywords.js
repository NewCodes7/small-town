...
// skip되어 정작 재려던 구간이 안 돈다. 그래서 실제 키워드를 조합해 "고유하면서도 문서에
// 매칭되는" 검색어를 만든다.
//
// term 개수(기본 2): SemanticTermExpansionService가 term마다 유의어를 DB 조회하므로 term 수가
// 늘수록 요청당 확장 쿼리가 비례해 늘어난다. 실사용 검색어는 보통 1~2 term이라 3-term은 DB
// 부하를 과대평가한다(2026-08-08 첫 캐시 미스 실행에서 확인). 고유성만 확보되면 되므로 2가 기본.
//
// 고유성 보장: exec.scenario.iterationInTest는 시나리오 내에서 유일하므로, 레벨마다 겹치지 않는
// 구간을 배정하면 테스트 전체에서 중복이 없다. 조합 수는 2-term이 101x100 = 10,100,
// 3-term이 101x100x99 = 999,900 — 레벨당 각각 2,525 / 249,975개까지 커버한다.
// 캐시 미스 모드는 요청량 자체가 적어(레벨당 수백 건) 2-term으로 충분하다.
const LEVEL_COUNT = 4;

export function uniqueKeyword(levelIndex, iterationInTest, terms = 2) {
  const n = keywords.length;
  const total = terms === 3 ? n * (n - 1) * (n - 2) : n * (n - 1);
  const slot = Math.floor(total / LEVEL_COUNT);
  const idx = (levelIndex * slot + iterationInTest) % total;

  // mixed radix 디코딩: 서로 다른 인덱스를 뽑는다
  const a = idx % n;
