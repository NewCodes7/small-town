...
### 선행 문서 7.4의 미해결에 대한 함의

R·S(13,500 blks/req) → T·U(2,100)는 이 모델로 설명될 여지가 크다.
`= 1`에서 대량 rebuild(`ArticleTermService:399`)가 한 번 지나가면 **행마다 세그먼트가 생겨 수천 개**가
되고(런북이 정확히 이 위험을 경고했다), 이후 백그라운드 병합이 이를 **균등한 4개로 재편**한다 —
08-19에 관측한 `4,565 / 4,585 / 4,870 / 4,565`가 바로 그 모양이다.
**다만 그 시점 세그먼트 기록이 없어 여전히 확정은 아니다.** 이번에 붙인 `bm25_index_segments`
시계열이 다음 톱니를 잡아야 닫힌다.
...
- [ ] `bm25_index_docs_mutable` 배포 → `mut` 열이 채워진다
- [ ] 같은 사다리 반복 — 8~13%가 노이즈인지 실재인지 가른다 (같은 컨테이너·같은 시간대)
- [ ] `mut` 값과 `core-s/req`의 상관 확인. 0.23 ms/doc 모델이 맞으면 선형이어야 한다
