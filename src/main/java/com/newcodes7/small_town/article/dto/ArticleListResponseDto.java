...
public class ArticleListResponseDto implements ArticleResponseDto {
     ...
     * BM25 + Vector 검색 결과를 Normalized Score Fusion으로 결합한 최종 스코어
    ...
    protected ArticleListResponseDto(Article article, Double bm25Score, Double vectorScore, Double finalScore) {
        this(article, bm25Score, vectorScore, finalScore, null, null);
    }

    /**
     * 모든 검색 스코어와 좋아요 상태를 포함한 생성자 (rank 없음)
     */
    public ArticleListResponseDto(Article article, Double bm25Score, Double vectorScore, Double finalScore, Double timeDecayScore, Boolean isLiked) {
        this(article, bm25Score, vectorScore, finalScore, timeDecayScore, null, null, null, null, null, isLiked);
    }

    /**
     * 모든 검색 스코어, 순위, 좋아요 상태를 포함한 생성자 (정규화 점수 없음)
     */
    public ArticleListResponseDto(Article article, Double bm25Score, Double vectorScore, Double finalScore, Double timeDecayScore,
                                  Integer bm25Rank, Integer vectorRank, Boolean isLiked) {
        this(article, bm25Score, vectorScore, finalScore, timeDecayScore, bm25Rank, vectorRank, null, null, null, isLiked);
    }
