package com.newcodes7.small_town.global.concurrency;
...
/**
 * 경로별 유입 제어(admission control)의 공통 본체 — 동시 실행 수를 세마포어로 상한 짓고
 * 초과분은 호출자가 429로 흘려보낸다.
 *
 * <p><b>왜 레이트 리밋이 아니라 동시성 제한인가.</b> 레이트 리밋은 "요청이 얼마나 비싼지"를 모른다.
 * 동시성 제한은 그 자체로 부하 적응적이다 — 캐시 히트로 금방 끝나는 요청은 permit을 금방 놓아
 * 낮은 한도로도 초당 여러 건이 지나가고, 요청이 느려질수록 permit이 오래 물려 자동으로 조여진다.
 *
 ...
 * 그대로 요청 경로에 얹힌다. 막으려는 문제를 로그가 다시 만드는 셈이라, 관측은 전적으로 메트릭에 맡긴다.
 * (RAG 런 3에서 실측된 모양이다 — 붕괴 12분간 에러 로그 37,338줄, 초당 52줄.
 * load-test/results/2026-08-27-rag-ladder.md 5.3)
 *
 * <p><b>한도는 JVM 인스턴스별이다.</b> blue/green은 nginx가 한쪽으로만 트래픽을 보내므로 정상 운영에서는
 * 실효 한도와 같지만, 전환 구간이나 이후 스케일아웃 시에는 인스턴스 수만큼 곱해진다는 점에 유의할 것.
 *
 * <p>한도는 DB(search_concurrency_config)의 {@code scope_name} 행에서 읽고 운영 중 변경할 수 있다.
 * DB 로드가 실패해도 서브클래스가 준 DEFAULTS로 계속 동작한다 — 설정 테이블 하나 때문에 서비스가
 * 죽으면 안 된다.
 *
 * <p>서브클래스는 스코프 이름·메트릭 접두사·기본값 셋만 다르다
 * ({@code SearchConcurrencyLimiter}, {@code RagConcurrencyLimiter}).
 */
@Slf4j
public abstract class ConcurrencyLimiter {
    ...
    /** DB에서 한도를 찾을 때 쓰는 scope_name 값. */
    private final String scopeName;
    /** 메트릭 이름 접두사 (예: {@code search_concurrency}). */
    private final String metricPrefix;
    /** 로그·메트릭 설명에 쓰는 사람이 읽는 이름 (예: {@code 검색}). */
    ...
    protected ConcurrencyLimiter(
            SearchConcurrencyConfigRepository repository,
        ...
        this.semaphore = new AdjustableSemaphore(defaults.maxConcurrent());
    ...
    public void init() {
        ...
        acceptedCounter = Counter.builder(metricPrefix + "_requests_total")
                .tag("result", "accepted")
                .description(displayName + " 동시성 제한을 통과한 요청 수")
                .register(meterRegistry);
        rejectedCounter = Counter.builder(metricPrefix + "_requests_total")
                .tag("result", "rejected")
                .description(displayName + " 동시성 제한에 걸려 429로 거절된 요청 수")
                .register(meterRegistry);
        acquireWaitTimer = Timer.builder(metricPrefix + "_acquire_wait")
                .description("permit 획득까지 대기한 시간(거절된 요청 포함)")
                .register(meterRegistry);
        Gauge.builder(metricPrefix + "_in_use", semaphore,
                        s -> (double) (s.total() - s.availablePermits()))
                .description("현재 사용 중인 permit 수")
                .register(meterRegistry);
        Gauge.builder(metricPrefix + "_limit", semaphore, s -> (double) s.total())
    ...
    /**
     * permit 획득을 시도한다. true를 돌려받은 호출자는 <b>반드시</b>
    ...
    public void updateLimits(int maxConcurrent, int acquireTimeoutMs, String updatedBy) {
        ...
        SearchConcurrencyConfig config = repository.findByScopeName(scopeName)
                .orElseGet(() -> SearchConcurrencyConfig.builder().scopeName(scopeName).build());
        ...
        log.info("[{} 동시성] 한도 업데이트: maxConcurrent={}, acquireTimeoutMs={}",
                displayName, maxConcurrent, acquireTimeoutMs);
    }

    private void refreshCache() {
        try {
            Limits loaded = repository.findByScopeName(scopeName)
                    .map(c -> new Limits(c.getMaxConcurrent(), c.getAcquireTimeoutMs()))
                    .orElse(defaults);
            applyLimits(loaded);
            log.info("[{} 동시성] 설정 로드 완료: maxConcurrent={}, acquireTimeoutMs={}",
                    displayName, loaded.maxConcurrent(), loaded.acquireTimeoutMs());
        } catch (Exception e) {
            // 설정 테이블을 못 읽었다고 서비스가 죽으면 안 된다 — 폴백으로 계속 간다
            log.error("[{} 동시성] DB 로드 실패, 기본값 유지(maxConcurrent={}): {}",
                    displayName, limits.maxConcurrent(), e.getMessage());
        }
    }
