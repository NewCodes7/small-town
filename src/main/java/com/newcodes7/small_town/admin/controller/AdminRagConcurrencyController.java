...
/**
 * RAG 동시 실행 상한 조회/변경 (AdminSearchConcurrencyController와 같은 패턴).
 * 화면은 admin/search-weights 페이지 안의 섹션으로 함께 제공된다 — 유입 제어/차단기 같은
 * "한도" 조작을 한 페이지에 모아 둔다.
 */
@Controller
@RequestMapping("/admin/rag/concurrency")
@RequiredArgsConstructor
@Slf4j
public class AdminRagConcurrencyController {
    ...
    public ResponseEntity<Map<String, Object>> getLimits() {
        ...
        try {
            RagConcurrencyLimiter.Limits limits = ragConcurrencyLimiter.getLimits();
            response.put("success", true);
            response.put("maxConcurrent", limits.maxConcurrent());
            response.put("acquireTimeoutMs", limits.acquireTimeoutMs());
            response.put("inUse", ragConcurrencyLimiter.getInUse());
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            log.error("[RAG 동시성] 조회 중 오류 발생", e);
    ...
    public ResponseEntity<Map<String, Object>> updateLimits(@RequestBody UpdateLimitsRequest request) {
        Map<String, Object> response = new HashMap<>();
        try {
            ragConcurrencyLimiter.updateLimits(
            ...
            log.error("[RAG 동시성] 업데이트 중 오류 발생", e);
            response.put("success", false);
