...
function setupDefaultThumbnails() {
    const icons = ['fa-code', 'fa-laptop-code', 'fa-terminal', 'fa-database', 'fa-server', 'fa-cogs', 'fa-microchip', 'fa-rocket', 'fa-chart-line', 'fa-lightbulb'];
    const texts = ['TECH', 'CODE', 'DATA', 'API', 'WEB', 'DEV', 'AI', 'ML', 'UI', 'UX'];
    ...
    document.querySelectorAll('.default-thumbnail').forEach((thumbnail, index) => {
        ...
        if (iconElement && textElement) {
            iconElement.className = `fas ${icons[index % icons.length]}`;
            textElement.textContent = texts[index % texts.length];
        }
    });
}
